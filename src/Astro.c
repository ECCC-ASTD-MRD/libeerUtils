...
#define Epsilon_g    (DEG2RAD(279.403303))
#define OmegaBar_g   (DEG2RAD(282.768422))
 ...
 */
#define MoonMeanLongitude        DEG2RAD(318.351648)
#define MoonMeanLongitudePerigee DEG2RAD( 36.340410)
#define MoonMeanLongitudeNode    DEG2RAD(318.510107)
#define MoonInclination          DEG2RAD(  5.145396)
 ...
 */
double solve_keplers_equation(double M) {
...
double mean_sun(double D) { /* days since ephemeris epoch */
 ...
 * (after duffett-smith, section 47) from seconds since unix epoch
 */
 double sun_ecliptic_longitude(time_t ssue) {
 ...
 */
 double GST(time_t ssue) {
   ...
   JD =System_Date2Julian(tm->tm_year+1900, tm->tm_mon+1, tm->tm_mday);
   T  = (JD - 2451545) / 36525;
...
void Astro_MoonPos(time_t ssue,float *lat,float *lon) {
   ...
   Normalize(L);
   Mm = L - DEG2RAD(0.1114041*D) - MoonMeanLongitudePerigee;
   Normalize(Mm);
   N = MoonMeanLongitudeNode - DEG2RAD(0.0529539*D);
   Normalize(N);
   Ev  = DEG2RAD(1.2739) * sin(2.0*(L-lambda)-Mm);
   Ae  = DEG2RAD(0.1858) * sin(Ms);
   Mm += Ev - Ae - DEG2RAD(0.37)*sin(Ms);
   Ec  = DEG2RAD(6.2886) * sin(Mm);
   L  += Ev + Ec - Ae + DEG2RAD(0.214) * sin(2.0*Mm);
   L  += DEG2RAD(0.6583) * sin(2.0*(L-lambda));
   N  -= DEG2RAD(0.16) * sin(Ms);
