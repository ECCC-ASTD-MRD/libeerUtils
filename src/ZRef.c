   ...
   if (key>=0) {
      ...
      if (cd>=0) {
         ...
         if (cd>=0) {
            ...
            Lib_Log(APP_LIBEER,APP_WARNING,"%s: Could not read !! field (c_fstluk).\n",__func__);
         }
      } else {
         Lib_Log(APP_LIBEER,APP_WARNING,"%s: Could not get info on !! field (c_fstprm).\n",__func__);
      }
   } else {
...
//            ZRef->Type=LVL_HYBRID;
            ...
            Lib_Log(APP_LIBEER,APP_WARNING,"%s: Could not get info on HY field (c_fstprm).\n",__func__);
         }
         // It might be ETA
         ...
         if (ZRef->Type==LVL_SIGMA || (ZRef->Type==LVL_ETA && ZRef->Version==-1)) {
            ...
            if (key>=0) {
               ...
               if (!(pt=(float*)malloc(h.NI*h.NJ*h.NK*sizeof(float)))) {
                  Lib_Log(APP_LIBEER,APP_WARNING,"%s: Could not allocate memory for top pressure, using default PTOP=10.0.\n",__func__);
               } else {
                  cd=c_fstluk(pt,key,&h.NI,&h.NJ,&h.NK);
                  if (cd>=0) {
                     ZRef->PTop=pt[0];
                  } else {
                     Lib_Log(APP_LIBEER,APP_WARNING,"%s: Could not read PT field (c_fstluk), using default PTOP=10.0.\n",__func__);
                  }
               }
            }
         }
