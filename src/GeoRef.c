   ...
   if (Ref->Grid[0]=='M') {
      ...
      if (!Ref->Wght) {
         if (!(Ref->Wght=(double*)calloc(Ref->NIdx/3,sizeof(double)))) {
            Lib_Log(APP_LIBEER,APP_WARNING,"%s: Failed to allocate baricentric weight array\n",__func__);
         }
      }
      
      // Create the tree on the data limits
      if (!(Ref->QTree=QTree_New(lon0,lat0,lon1,lat1,NULL))) {
         Lib_Log(APP_LIBEER,APP_ERROR,"%s: Failed to create QTree index\n",__func__);
         return(NULL);
      }
      ...
      for(n=0,t=0;n<Ref->NIdx;n+=3,t++) {          
         ...
         if (!QTree_AddTriangle(Ref->QTree,tr,GRID_MQTREEDEPTH,(void*)(n+1))) {
            Lib_Log(APP_LIBEER,APP_ERROR,"%s: Failed to add node\n",__func__);
            return(NULL);
         }      
      }
...
int GeoRef_Valid(TGeoRef* __restrict const Ref) {
   ...
   if (co[0].Lat<-91 || co[0].Lat>91.0 || co[1].Lat<-91 || co[1].Lat>91.0) {
      Lib_Log(APP_LIBEER,APP_DEBUG,"%s: Limits: %f %f - %f %f\n",__func__,co[0].Lat,co[0].Lon,co[1].Lat,co[1].Lon);
      return(0);
   }
   return(1);
}
...
//   RPN_IntLock();
   if (!Ref || Ref->Grid[0]=='X' || Ref->Grid[0]=='Y') {
      Lib_Log(APP_LIBEER,APP_WARNING,"%s: DX, DY and DA cannot be calculated on an X or Y grid\n",__func__);        
      return(FALSE);
   } else if (Ref->Grid[0]=='M') {
      
      if (DX || DY) {
         Lib_Log(APP_LIBEER,APP_WARNING,"%s: DX and DY cannot be calculated on an M grid\n",__func__);        
      }
