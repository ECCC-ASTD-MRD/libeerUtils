   ...
   if (GRef->Type&GRID_SPARSE) {
       if (GRef->AX && GRef->AY) {
         if (GRef->Grid[0]=='Y' || GRef->Grid[0]=='M') {
            ...
            dy=Vertex_ValS(GRef->AY,NULL,GRef->NX,GRef->NY,X,Y,TRUE);
...
TGeoRef* GeoRef_RPNSetup(int NI,int NJ,char *GRTYP,int IG1,int IG2,int IG3,int IG4,int FID) {
   ...
   if ((NI>1 || NJ>1) && GRTYP[0]!='X' && GRTYP[0]!='O' && GRTYP[0]!='P' && GRTYP[0]!='M' && GRTYP[0]!='R' && GRTYP[0]!='V' && GRTYP[0]!='Y' && (GRTYP[0]!='Z' || FID!=-1)) {
      grtyp[0]=GRTYP[0];
      grtyp[1]='\0';
