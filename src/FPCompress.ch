...
static void FPC_PutByte(TFPCCtx *restrict Ctx) {
    ...
    if( Ctx->FD!=-1 && Ctx->BufCnt>=FPC_BUF_SIZE ) {
        if( write(Ctx->FD,Ctx->CData,Ctx->BufCnt) != Ctx->BufCnt ) {
            Lib_Log(APP_LIBEER,APP_ERROR,"Problem while writing the data : field will be corrupted\n");
        }
        Ctx->BufCnt = 0;
    }
}
...
int R(FPC_Compress)(void *restrict CData,int FD,TFPCReal *restrict Data,int NI,int NJ,int NK,size_t *restrict CSize) {
    ...
    if( NI<1 || NJ<1 || NK<1 ) {
        Lib_Log(APP_LIBEER,APP_ERROR,"Dimensions must be greater or equal to 1\n");
        return APP_ERR;
    }

    // Make sure we have a chance to compress stuff
    //if( n < 16 ) {
    //    Lib_Log(APP_LIBEER,APP_ERROR,"Compression of a field of less than 16 values is a waste of time\n");
    //    return APP_ERR;
    //}

    // If a file was given, use a write buffer instead
    if( FD!=-1 && !(CData=malloc(FPC_BUF_SIZE)) ) {
        Lib_Log(APP_LIBEER,APP_ERROR,"Could not allocate memory for write buffer\n");
        return APP_ERR;
    }

    if( !(ctx=FPC_New(CData,FD)) ) {
        Lib_Log(APP_LIBEER,APP_ERROR,"Could not allocate memory for context\n");
        if( FD!=-1 )
            free(CData);
        return APP_ERR;
    }
    ...
    Lib_Log(APP_LIBEER,APP_DEBUG,"Compressing %zu values, flag=%d ndims=%u d1=%zu d2=%zu bufs=%zu\n",n,flag,ndims,d1,d2,bufs);
    ...
    if( ctx->Cnt < maxcnt ) {
       ...
       if( ctx->Cnt <= n ) {
           *CSize = ctx->Cnt;
           Lib_Log(APP_LIBEER,APP_DEBUG,"Initial data size : %zu bytes. New data size : %zu bytes. Compression factor : %.2f\n",n,ctx->Cnt,(double)(n)/(double)ctx->Cnt);
       } else {
           Lib_Log(APP_LIBEER,APP_WARNING,"Compression was aborted because the size of the compressed data would have been over the inital size.\n");
           code = APP_ERR;
       }
    } else {
       Lib_Log(APP_LIBEER,APP_WARNING,"Compression was aborted because we were going to be over the resulting buffer size, which means the compression wasn't working very well on this dataset anyway.\n");
       code = APP_ERR;
    }
...
int R(FPC_Inflate)(void *restrict CData,int FD,TFPCReal *restrict Data,int NI,int NJ,int NK) {
    ...
    if( FD != -1 ) {
        ...
        if( (off=lseek(FD,0,SEEK_CUR)) == -1 ) {
            Lib_Log(APP_LIBEER,APP_ERROR,"Could not tell position in the file : %s\n",strerror(errno));
            return APP_ERR;
        }
        ...
        if( (CData=mmap(NULL,mxtra+n*sizeof(*Data),PROT_READ,MAP_SHARED,FD,poff)) == MAP_FAILED ) {
            Lib_Log(APP_LIBEER,APP_ERROR,"Could not map file to decompress data : %s\n",strerror(errno));
            return APP_ERR;
        }
    }
    ...
    Lib_Log(APP_LIBEER,APP_DEBUG,"Inflating %zu values, flag=%d ndims=%u d1=%zu d2=%zu bufs=%zu\n",n,flag,ndims,d1,d2,bufs);
    ...
    Lib_Log(APP_LIBEER,APP_DEBUG,"Inflated size : %zu bytes. Read : %zu bytes\n",n*sizeof(*Data),ctx->Cnt);
